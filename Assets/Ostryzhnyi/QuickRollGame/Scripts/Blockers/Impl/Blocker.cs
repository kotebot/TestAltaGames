...
namespace Ostryzhnyi.QuickRollGame.Scripts.Blockers.Impl
{
    ...
    {
        [SerializeField] private List<StateByObject> _states = new();
        [SerializeField] private LayerMask _mask;

        [SerializeField] [Range(0.01f, 2f)] private float _delayBeforeChangeState = .1f;
        ...
        private IEnumerator ExplosionRoutine(float radius)
        {
            ...
            foreach (var blocker in neighbors) blocker.SetState(BlockerState.Removing);

            yield return new WaitForSeconds(_delayBeforeChangeState);

            SetState(BlockerState.Removed);

            foreach (var blocker in neighbors) blocker.SetState(BlockerState.Removed);
        }
        ...
        {
            foreach (var state in _states) state.Object.SetActive(false);
        }
    }
