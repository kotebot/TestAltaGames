...
namespace Ostryzhnyi.QuickRollGame.Editor
{
    public class LevelGeneratorWindow : EditorWindow
    {
        ...
        {
            if (_levelGenerator == null) Initialize();
        }
        ...
        private void DrawSettings()
        {
            ...
            _levelGenerator.CenterElement = (Transform)EditorGUILayout.ObjectField("Center Element",
                _levelGenerator.CenterElement, typeof(Transform), true);
            _levelGenerator.BlockerPrefab = (GameObject)EditorGUILayout.ObjectField("Blocker Prefab",
                _levelGenerator.BlockerPrefab, typeof(GameObject), false);
        ...
        {
            EditorGUILayout.Space();
            _levelGenerator.LevelData =
                (LevelData)EditorGUILayout.ObjectField("Level Data", _levelGenerator.LevelData, typeof(LevelData),
                    false);
            ...
            if (_levelGenerator.LevelData.Levels.Count > 0)
                _levelGenerator.SelectedLevelIndex = EditorGUILayout.IntSlider("Select Level",
                    _levelGenerator.SelectedLevelIndex, 0, _levelGenerator.LevelData.Levels.Count - 1);
            EditorGUILayout.BeginHorizontal();
        ...
        private void LoadSettings()
        {
            ...
            if (_levelGenerator.BlockerPrefab == null)
            {
                var prefab =  AssetDatabase.LoadAssetAtPath<GameObject>(
                    "Assets/Ostryzhnyi/QuickRollGame/Scripts/Blockers/Impl/Prefabs/Blocker.prefab");
                _levelGenerator.BlockerPrefab = prefab;
            ...
            _levelGenerator.LevelData =
                AssetDatabase.LoadAssetAtPath<LevelData>(
                    "Assets/Ostryzhnyi/QuickRollGame/Scripts/Map/Data/LevelData.asset");
        ...
        private void SaveSettings()
        {
            ...
            {
                var assetPath = AssetDatabase.GetAssetPath(_levelGenerator.LevelData);
                EditorPrefs.SetString("LG_LevelDataPath", assetPath);
            }
        }

        private T LoadSetting<T>(string key) where T : Object
        {
            return EditorPrefs.HasKey(key) ? EditorUtility.InstanceIDToObject(EditorPrefs.GetInt(key)) as T : null;
        }

        private void SaveSetting<T>(string key, T obj) where T : Object
        {
            if (obj != null) EditorPrefs.SetInt(key, obj.GetInstanceID());
        }
